...
BinaryHeap.prototype._getLesserChildIndex = function (parentIndex) {
  //use this method to get childIndex if it exists, otherwise return null.
  const childIndex = index => this._heap.length > index ? index : null;
  const leftNodeIndex = childIndex(parentIndex * 2 + 1);
  const rightNodeIndex = childIndex(parentIndex * 2 + 2);

  //if both indices exist, return the one with a lower value
  if (leftNodeIndex && rightNodeIndex) {
    return this._compare(this._heap[leftNodeIndex], this._heap[rightNodeIndex])
    ? leftNodeIndex
    : rightNodeIndex;

  //if both indices don't exist, return the one that does exist
  } else {
...
BinaryHeap.prototype.insert = function (value) {
  ...
  //while the child index is lesser than it's parent, swap
...
BinaryHeap.prototype.removeRoot = function () {
  //save root before it gets over-written
  const root = this.getRoot();
  //pop off the last item in the array and hold as a temp
  const temp = this._heap.pop();
  //if the heap still has a length after being popped, set the rootNode (this._heap[0]) to the temp
  if (this._heap.length) this._heap[0] = temp;

  let parentIndex = 0;
  let index = this._getLesserChildIndex(parentIndex);

  //while a child index exists  &&  the child index is less than the parentIndex
  while (index && this._compare(this._heap[index], this._heap[parentIndex])) {
    this._swap(index, parentIndex);
    parentIndex = index;
    index = this._getLesserChildIndex(parentIndex);
  }
  return root;
}
